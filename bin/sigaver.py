...
import optparse
from scipy import *
from numpy import array, hstack, shape, sqrt, sum, zeros, savetxt
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="osig",
        default="sig.inpx",
        help="filename of the output self-energy file. Default: 'sig.inp'",
    )
    parser.add_option(
        "-l",
        "--lext",
        dest="m_extn",
        default="",
        help="For magnetic calculation, it can be 'dn'.",
    )
    parser.add_option(
        "-n",
        "--nexecute",
        ...
        action="store_true",
        default=False,
        help="Do not execute the comments",
    )
    parser.add_option(
        "-s",
        dest="stdev",
        action="store_true",
        default=False,
        help="Computes also the standard deviation - the error of self-energy",
    )
    ...
    for f in args:
        ...
        for line in dat:
            ...
            if m is not None:
                ...
                if (
                    m.group(1).strip()[:5] == "s_oo="
                    or m.group(1).strip()[:5] == "Vdc= "
                ):
    ...
    if len(ws_oo):
        ws_oo = array(ws_oo)
        as_oo = []
        for i in range(shape(ws_oo)[1]):
            as_oo.append(sum(ws_oo[:, i]) / len(ws_oo))
        print("s_oo=", as_oo)
    ...
    if len(wVdc):
        wVdc = array(wVdc)
        aEdc = []
        for i in range(shape(wVdc)[1]):
            aEdc.append(sum(wVdc[:, i]) / len(wVdc))
        print("Vdc=", aEdc)
        print("# Vdc=", aEdc, file=fout)
