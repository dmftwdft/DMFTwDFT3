...
class WANNIER:
    ...
    def load_eig(self):
        if os.path.exists(self.name + ".eig"):
            print("eig file exists!")
        else:
            print("eig file does not exist! We are exiting")
            exit()
        fi_eig = open(self.name + ".eig", "r")
        eig_lines = fi_eig.readlines()
        eigvals = [float(line.split()[2]) for line in eig_lines]
        last_fields = eig_lines[-1].split()
        self.num_kpts = int(last_fields[1])
        self.num_bands = int(last_fields[0])
    ...
    def load_chk(self, path):
        ...
        if os.path.exists(self.name + ".chk.fmt"):
        ...
        print(("excl_bands : %s" % excl_bands))
        real_latt = list(map(float, fi_chk.readline().split()))
        print(("real_latt : %s" % real_latt))
        real_latt = array(real_latt).reshape((3, 3), order="F")
        recip_latt = list(map(float, fi_chk.readline().split()))
        self.recip_latt = array(recip_latt).reshape((3, 3), order="F")
        num_kpts = int(fi_chk.readline())
        self.num_kpts = num_kpts
        mp_grid = list(map(int, fi_chk.readline().split()))
        self.mp_grid = array(mp_grid)
        kpt_latt = [
            list(map(float, fi_chk.readline().split())) for i in range(num_kpts)
        ]
        self.kpt_latt = array(kpt_latt)
        self.ikpt = array(
            [
                list(map(int, list(map(round, array(mp_grid) * array(kp)))))
                % array(mp_grid)
                for kp in kpt_latt
            ]
        )
        ...
        if have_disentangled:
            ...
            u_matrix_opt = [
                list_to_complex(fi_chk.readline().split())
                for i in range(num_kpts)
        ...
        for nkp in range(num_kpts):
            IP = 0
            nbmin = num_tot_bands - 1
            nbmax = 0
            for nb in range(num_tot_bands):
                if lexclude_band[nb]:
                    continue
                IP += 1
                if not lwindow[nkp, IP - 1]:
                    continue
                if nb + 1 < nbmin:
                    nbmin = nb
                if nb + 1 > nbmax:
    ...
    def Compute_dU(self):
        ...
        if os.path.exists(self.name + ".amn"):
            print("amn file exists!")
        else:
            print("amn file does not exist! We are exiting")
            exit()
        ...
        num_proj = int(line[2])
        AMN = [
            list_to_complex(fi_amn.readline().split()[3:5])
            for i in range(num_kpts)
        ...
        ######## READ dAMN file #############
        if os.path.exists(self.name + ".damn"):
            print("damn file exists!")
        else:
            print("damn file does not exist! We are exiting")
            exit()
        ...
        self.num_fdir = num_dir * len(Atom)
        dAMN = [
            list_to_complex(fi_amn.readline().split()[4:6])
            for i in range(num_kpts)
            for j in range(num_proj)
            for k in range(num_bands)
            for l in range(num_dir)
        ]
        dAMN = array(dAMN).reshape(num_kpts, num_proj, num_bands, num_dir)
        fi_amn.close()

        self.dWANU = zeros(
            (num_kpts, num_proj, self.num_band_max, len(Atom) * num_dir), dtype=complex
        )
        for i in range(num_kpts):
            nbmin = self.band_win[i][0]
            nbmax = self.band_win[i][1]
            num_band_max = nbmax - nbmin + 1
            FAC = array(
                matrix(self.WANU[i][:, :num_band_max])
                * matrix(AMN[i][:, nbmin : nbmax + 1]).I
            )
            for ii in range(len(Atom)):
                for k in range(num_dir):
                    dU = zeros((num_proj, num_band_max), dtype=complex)
                    dU[Atom[ii][0] : Atom[ii][1], :] = dAMN[i][
                        Atom[ii][0] : Atom[ii][1], nbmin : nbmax + 1, k
                    ]
    ...
    def Compute_HamR0(self):
        ...
        for i in range(self.num_kpts):
            ...
            self.HamR0 += (
                Uwanband * diag(self.eigvals[i][nbmin : nbmax + 1]) * Uwanband.H
            )
...
if __name__ == "__main__":
    ...
    ##WAN.print_H0()
    exec(compile(open("rham.py", "rb").read(), "rham.py", "exec"))
