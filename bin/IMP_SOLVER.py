...
def CreateInputFile(params_ctqmc):
    ...
    for p in list(params_ctqmc.keys()):
        print(p, "\t", params_ctqmc[p][0], "\t", params_ctqmc[p][1], file=f)
    f.close()
...
def Create_atomd(params_atomd):
    ...
    f = open("atom_d.inp", "w")
    for p in list(params_atomd.keys()):
        if (
            p == "n="
            or p == "Ewindow="
            or p == "Ex="
            ...
            for elem in params_atomd[p]:
                print(str(elem) + ",", end=" ", file=f)
            print("]", file=f)
        elif p == "CoulombF=":
            print(p, '"' + params_atomd[p] + '"', file=f)
        else:
            print(p, params_atomd[p], file=f)
    f.close()
...
def Create_Trans(norb, nspin, at, cor_orbs, TB):
    ...
    f = open("Trans.dat", "w")
    print(norb * nspin, norb * nspin, " #  size of Sigind and CF", file=f)
    ...
    for i, orb in enumerate(d_orb):
        ...
        Trans = orb_Trans[orb]
        for elem in Trans:
            print("%10.8f %10.8f  " % (elem.real, elem.imag), end=" ", file=f)
        print("", file=f)
