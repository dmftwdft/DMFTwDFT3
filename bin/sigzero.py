...
def GiveTanMesh(x0, L, Nw):
    ...
    xi = x0 / L
    d0 = Nw / 2.0 * (tan(pi / (2 * Nw)) - sqrt(tan(pi / (2 * Nw)) ** 2 - 4 * xi / Nw))
    w0 = L * d0
...
def default_realaxis_mesh():
    ...
    args = ["x0=[0,0]", "dx0=[0.25,0.01]", "fwhm=[30, 3]", "xmin=-20", "xmax=20"]
    stdoutdata, stderrdata = subprocess.Popen(
        [gaumesh] + args, stdout=subprocess.PIPE
    ).communicate()
    stdout_text = stdoutdata.decode("utf-8")
    return [
        float(x)
        for x in stdout_text.split("\n")
...
if __name__ == "__main__":
    ...
    if os.path.exists(INPUT_FILE):
        ...
        for i, ats in enumerate(p["cor_at"]):
            for j, orbs in enumerate(p["cor_orb"][i]):
                if len(orbs) > 0:
                    nc += 1
        nc *= p["nspin"]
        Vdc = p["U"][0] * (p["nf"] - 0.5) - 0.5 * p["J"][0] * (p["nf"] - 1.0)
        T = 1.0 / pC["beta"][0]
        ...
        parser.add_option(
            ...
            type="int",
            default=nc,
            help="Number of correlated orbitals",
        )
        parser.add_option(
            "-e",
            "--Edc",
            ...
            type="float",
            default=Vdc,
            help="Starting double counting and Hartree value of the self-energy. Should be close to U(n-1/2)",
        )
        parser.add_option(
            "-i",
            "--sinp",
            dest="insig",
            default="sig.inp",
            help="the mesh will be used from this file.",
            metavar="FILE",
        )
        parser.add_option(
            "-o",
            ...
            dest="outsig",
            default="sig.inp",
            help="the result will be written to this file",
        )
        parser.add_option(
            "-T", "--Temp", dest="T", type="float", default=T, help="Temperature"
        )
        parser.add_option(
            "-n",
            "--nom",
        ...
        parser.add_option(
            ...
            type="float",
            default=20.0,
            help="energy range on real axis",
        )
        parser.add_option(
            "-x",
            "--x0",
        ...
        parser.add_option(
            ...
            type="int",
            default=400,
            help="Number of frequency points on real axis",
        )
        ...
        parser.add_option(
            "-T", "--Temp", dest="T", type="float", default=0.0, help="Temperature"
        )
        parser.add_option(
            "-n",
            "--nom",
    ...
    if options.nom == None and options.T > 0:
        options.nom = int((300.0 / options.T - 1) / (2.0 * pi)) + 1
    else:
        options.nom = 1
    ...
    if p["nspin"] == 1:
        ...
        print(
            "# s_oo=",
            (ones(half_nc) * (options.Edc - Emag)).tolist()
            + (ones(half_nc) * (options.Edc + Emag)).tolist(),
            file=fo,
        )
