...
from scipy import *
from numpy import array, loadtxt, log10, savetxt, sqrt, sum, zeros
...
class VASP_class:
            ...
            for j in range(idx / 5):
                ...
                for k in range(j * 5, (j + 1) * 5):
                    if augCHG[i][k] >= 0:
                        ...
                        print("-%.7E" % abs(augCHG[i][k]), end=" ", file=fi)
                print("", file=fi)
            if idx % 5 > 0:
                print("", end=" ", file=fi)
                for j in range((idx / 5) * 5, idx):
                    if augCHG[i][j] >= 0:
                        ...
                        print("-%.7E" % abs(augCHG[i][j]), end=" ", file=fi)
                print("", file=fi)
        fi.close()
    ...
    def Create_win(
        ...
        print("dis_num_iter   = " + str(dis_num_iter), file=fi)
        print("", file=fi)
        ...
        for i, at in enumerate(atomnames):
            if L_rot[i] == 1:
                for j in range(TB.num_atoms[at]):
                    ...
                    print(
                        "f=" + cordi + ":l=" + str(TB.L[orbs[i]]) + ":" + rot_vec,
                        file=fi,
                    )
            elif L_rot[i] == 0:
                print(at + ":" + str(orbs[i]), file=fi)
            else:
