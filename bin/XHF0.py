...
from numpy import arange, array, diag, dot, matrix, ones, shape, sum, transpose, zeros
...
def HF_eigsystem(KPTS, TOT_NKPTS, NBANDMAX, EIGVAL, BAND_WIN, WANU):
    ...
    for ikp, kp in enumerate(KPTS):
        if kp < TOT_NKPTS + 1:
            nbmin = BAND_WIN[ikp][0]
            nbmax = BAND_WIN[ikp][1]
...
def Ksum_HF(KPTS, TOT_NKPTS, NBANDMAX, NWANN, EIGVAL, BAND_WIN, WANU, evalHF, KWEIGHT):
    ...
    for ikp, kp in enumerate(KPTS):
        if kp < TOT_NKPTS + 1:
            ...
            EKIN += (
                2
                * dot(EIGVAL[ikp][nbmin : nbmax + 1], KWEIGHT[ikp][:num_band_max]).real
            )
            # EKIN+=dot(EIGVAL[ikp][nbmin:nbmax+1],diag(Uwanband2*DMFTW_perk*Uwanband2.H)).real
            DM += (
                2 * diag(Uwanband * diag(KWEIGHT[ikp][:num_band_max]) * Uwanband.H).real
            )
    return DM, EKIN
...
def Get_Tetra(recip_latt, nk, ikpt):
    ...
    mdiag = sorted(
        [
            (sum(dot(vec, array(verts[ss[0]]) - array(verts[ss[1]])) ** 2), s)
            for s, ss in enumerate(diags)
        ]
    )[0][-1]
    ...
    for i, ii in enumerate(temp):
        for j, jj in enumerate(temp[i:]):
            if sum((array(ii) - array(jj)) ** 2) == 1:
                ptet.append(sorted([mdiag[0], mdiag[1], ii, jj]))
    nk = tuple(nk)
...
def Compute_TETRA(
    ef, TOT_NKPTS, NBANDMAX, tetkptr, tet_idx, evalHF_reciv, TET_KPTS, ntet
):
    ...
    for ikp, kp in enumerate(TET_KPTS):
        if kp < ntet + 1:
            ...
            for b in range(NBANDMAX):
                ...
                order = list(map(int, array(sorted(Elist))[:, 1]))
                ...
                if ef > e1 and ef < e2:
                    ...
                    cw1 = c * (
                        4 - (ef - e1) * (1 / (e2 - e1) + 1 / (e3 - e1) + 1 / (e4 - e1))
                    )
                    cw2 = c * (ef - e1) / (e2 - e1)
                    cw3 = c * (ef - e1) / (e3 - e1)
                    cw4 = c * (ef - e1) / (e4 - e1)
                    ...
                    cw3 = (c1 + c2) * (ef - e1) / (e3 - e1) + (c2 + c3) * (ef - e2) / (
                        e3 - e2
                    )
                    cw4 = (c1 + c2 + c3) * (ef - e1) / (e4 - e1) + c3 * (ef - e2) / (
                        e4 - e2
                    )
                    ...
                    cw4 = 0.25 - c * (
                        4 - (1 / (e4 - e1) + 1 / (e4 - e2) + 1 / (e4 - e3)) * (e4 - ef)
                    )
                elif ef > e4:
                    cw1 = 0.25
                    cw2 = 0.25
...
def Compute_EPOT(DM, MOM, ncor_orb, norb, atm_idx, U, Up, Jh):
    ...
    for i, ats in enumerate(atm_idx):
        ...
        OCC = array(
            list((DM[norb * i : norb * (i + 1)] + MOM[norb * i : norb * (i + 1)]) / 2)
            + list((DM[norb * i : norb * (i + 1)] - MOM[norb * i : norb * (i + 1)]) / 2)
        )
...
if __name__ == "__main__":
    ...
    if os.path.exists("dmft_params.dat"):
        ...
        n_orbs = int(float(fi.readline().strip()))
    else:
        print("dmft_params.dat is missing")
        exit()
    ...
    if rank == 0:
        ...
        NWANN = WAN.num_wann
        NEXBANDS = WAN.num_exclude_bands
        ...
        if NEXBANDS > 0:
            WANU_send = zeros(
                (len(KPTS_send), NWANN, (NBANDMAX - NEXBANDS)), dtype=complex
            )
    ...
    for i in range(mu_iter):
        ...
        TET_KWEIGHT = Compute_TETRA(
            mu, TOT_NKPTS, NBANDMAX, tetkptr, tet_idx, evalHF_reciv, TET_KPTS, ntet
        )
        comm.Reduce(TET_KWEIGHT, TET_KWEIGHT_sum, op=MPI.SUM)
        if rank == 0:
            KWEIGHT_send[:TOT_NKPTS, :] = TET_KWEIGHT_sum[:, :]
    ...
    DM, EKIN = Ksum_HF(
        KPTS, TOT_NKPTS, NBANDMAX, NWANN, EIGVAL, BAND_WIN, WANU, evalHF, KWEIGHT
    )
    ...
    if rank == 0:
        ...
        HF_iter.write(
            ...
            % (
                ...
                totN,
                sum(DM_sum[:n_orbs]),
                sum(DM_sum[n_orbs * (Natom - 1) : n_orbs * Natom]),
                EKIN,
                0.0,
            )
        )
