...
def Make_orthogonal(Egval, EgvecL, EgvecR, smalleps=1e-5):
    for i in range(len(Egval)):
        for j in range(i):
            if (
                abs(Egval[i] - Egval[j]) < smalleps
                and dot(conjugate(EgvecL[:, j]), EgvecR[:, i]) > smalleps
            ):
                print("EigenvectorR are not orthogonal!")
                EgvecR[:, i] = (
                    EgvecR[:, i]
                    - dot(conjugate(EgvecL[:, j]), EgvecR[:, i])
                    / dot(conjugate(EgvecL[:, j]), EgvecR[:, j])
                    * EgvecR[:, j]
                )
        for j in range(i):
            if (
                abs(Egval[i] - Egval[j]) < smalleps
                and dot(conjugate(EgvecL[:, i]), EgvecR[:, j]) > smalleps
            ):
                print("EigenvectorL are not orthogonal!")
                EgvecL[:, i] = (
                    EgvecL[:, i]
                    - dot(EgvecL[:, i], conjugate(EgvecR[:, j]))
                    / dot(EgvecL[:, j], conjugate(EgvecR[:, j]))
                    * EgvecL[:, j]
                )
