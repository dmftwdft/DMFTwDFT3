...
from numpy import array, cross, dot, matrix, sqrt, zeros
...
class TBstructure:
    def __init__(self, poscar, atomnames, orbs):
        ...
        for ia, atname in enumerate(atomnames):
            ...
            if chkpoint == 0:
                ...
                exit()
        self.natoms = len(list(self.TB_orbs.keys()))
    ...
    def Rot_axis(self, rot_at):
        ...
        del comp[-1]
        comp.sort(key=lambda item: abs(item[1]))
        for cp in comp:
            n1 = array(cp[1:]) / cp[0]
        ...
        # print 'Atom=',rot_at
        return (
            "%.8f,%.8f,%.8f" % (self.atd[at1][0], self.atd[at1][1], self.atd[at1][2]),
            "z=%.8f,%.8f,%.8f:x=%.8f,%.8f,%.8f"
            % (n3[0], n3[1], n3[2], n_1[0], n_1[1], n_1[2]),
        )
...
def cmp_list(x, idx=3):
    return abs(x[idx])


def cmp_list2(x, idx=1):
    return abs(x[idx])
