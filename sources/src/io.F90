...
contains
  subroutine write_float(filename,float_data)

    implicit none

    character(len=*), intent(in) :: filename 
    real(kind=dp), intent(in) :: float_data(:)
    integer :: dim1, dim2, im, ib, ierr
    
    dim1=size(float_data)
    !write(*,*) 'aa',dim1,float_data
    OPEN(UNIT=90,FILE=filename,FORM='FORMATTED',iostat=ierr)
    !DO ib=1,dim1
    !   WRITE(90,'(F20.15)',advance='no') float_data(ib) 
    !ENDDO
    !write(*,*) ierr
    DO ib=1,dim1-1
    !  write(*,*) 'bb'
      WRITE(90,'(F20.15)',advance='no') float_data(ib)
    ENDDO
    WRITE(90,'(F20.15)') float_data(dim1)
    CLOSE(90)
  end subroutine write_float

  subroutine write_complex_function(filename,cmplx_data,mesh)

    implicit none

    character(len=*), intent(in) :: filename 
    complex(kind=dp), intent(in) :: cmplx_data(:,:)
    real(kind=dp), intent(in) :: mesh(:)
    integer :: dim1, dim2, im, ib
    
    dim1=size(cmplx_data,1); dim2=size(cmplx_data,2)
    !write(*,*) dim1,dim2
    !write(*,*) cmplx_data(1,1)
    OPEN(UNIT=99,FILE=filename,FORM='FORMATTED')
    DO im=1,dim2
      WRITE(99,'(F20.15)',advance='no') mesh(im)
      DO ib=1,dim1-1
        WRITE(99,'(2F20.15)',advance='no') real(cmplx_data(ib,im)),aimag(cmplx_data(ib,im))
      ENDDO
      WRITE(99,'(F20.15)',advance='no') real(cmplx_data(dim1,im))
      WRITE(99,'(F20.15)') aimag(cmplx_data(dim1,im))
    ENDDO
    CLOSE(99)
  end subroutine write_complex_function
