...
!      allocate(Sig_st(2*ncor_orb))
     ...
     1      MPI_SUM,MPI_Comm_World,ierr)
      ...
      if (rank.eq.0) then
        !write(*,*) 2*num_wann+1
        allocate(write_Gloc(2*num_wann+1,nom,nspin))
        do ispin=1,nspin
          do im=1,nom
 ...
 110    FORMAT('(',I2,'F18.12)')
        WRITE(write_format,110) 2*ncor_orb+1!2*num_wann+1
        !write(*,*) write_format
        DO im=1,nom
 ...
 111      FORMAT('(',I2,'F18.12)')
          WRITE(write_format,111) 2*num_wann+1
          !write(*,*) write_format
          DO im=1,nom
            WRITE(99,write_format) (write_Gloc(i,im,2),i=1,2*num_wann+1)
          ENDDO
          CLOSE(99)
        endif
