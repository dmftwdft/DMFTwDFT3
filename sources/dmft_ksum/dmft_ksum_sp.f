...
!      enddo
      ...
      if (nspin .eq. 2) then
         inquire(file='SigMoo_dn.out',exist=iffile)
         if (iffile .eqv. .true.) then
            open(unit=20,file='SigMoo_dn.out',form='formatted')
            do j=1,nom
               read(20,*) (SigMdc(i,j,2),i=1,2*ncor_orb+1)
            enddo
            close(20)
         else
            write(*,*) 'SigMoo_dn.out file must exist!!!'
            STOP
         endif
      endif
...
!      allocate(Sig_st(2*ncor_orb))
      allocate(tot_Ed(ncor_orb))
      allocate(tot_dm(num_wann))
      allocate(tot_mom(num_wann))
      allocate(tot_Gloc(num_wann,nom,nspin))
      do iter=1,Niter 
        allocate(hk(num_wann,num_wann))
        allocate(evec0(num_wann,num_wann))
        allocate(eval0(num_wann))
        allocate(t_eval0(num_wann,nspin,numk))
        allocate(t_eval(num_wann,noms,nspin,numk))
        ...
        do nkp=1,num_kpts
          ...
          if (ikp.gt.0) then
            ...
            do ispin=1,nspin
              ...
              evec0=hk*(1.0)
              do i=1,ncor_orb
                evec0(i,i)=evec0(i,i)+SigMdc(2*i,nom,ispin)
              enddo
              CALL EIGVALH(num_wann,evec0,eval0) 
              do nb=1,num_wann
                t_eval0(nb,ispin,ikp)=eval0(nb)
              enddo
              do im=1,noms
                evec0=0;evalue=0
                evec0=hk*(1.0) 
                do i=1,ncor_orb
                  evec0(i,i)=evec0(i,i)
     1            +dcmplx(SigMdc(2*i,im,ispin),SigMdc(2*i+1,im,ispin))
                enddo
                CALL EIGVAL(num_wann,evec0,evalue,evl,evr)
                do nb=1,num_wann
                  t_eval(nb,im,ispin,ikp)=evalue(nb)
                enddo
              enddo
            enddo
          endif
        ...
        do l=1,mu_iter 
          ...
          do nkp=1,num_kpts
            ...
            if (ikp.gt.0) then
              do ispin=1,nspin
                do nb=1,num_wann
                  tot_n=tot_n+2*fermi((t_eval0(nb,ispin,ikp)-mu)/T)
     1                  *sweight*kweight(nkp)
                  do im=1,noms
                    Gs0=1.0/(mu+dcmplx(0.0,SigMdc(1,im,ispin))
     1                  -t_eval0(nb,ispin,ikp))
                    Gs=1.0/(mu+dcmplx(0.0,SigMdc(1,im,ispin))
     1                  -t_eval(nb,im,ispin,ikp))
                    tot_n=tot_n+4*T*real(Gs-Gs0)*kweight(nkp)*sweight
                  enddo
                enddo
              enddo
            endif
     ...
     1          MPI_SUM,MPI_Comm_World,ierr)
        ...
        Ed=0;Gloc=0;Ekin=0;mom=0;dm=0;IPIV=0
        do nkp=1,num_kpts
          ikp = nkp-rank*pr_proc
          if (ikp.GT.numk) EXIT
          if (ikp.gt.0) then
     ...
     1            +dcmplx(SigMdc(2*i,im,ispin),SigMdc(2*i+1,im,ispin))
                ...
                do nb=1,num_wann
                  Gs0=1.0/(mu+dcmplx(0.0,SigMdc(1,im,ispin))-eval0(nb))
                  Gs=1.0/(mu+dcmplx(0.0,SigMdc(1,im,ispin))-evalue(nb))
                  do i=1,num_wann
                    do j=1,num_wann
                      DMFT_UU=evr(i,nb)*evl(nb,j)*Gs
                      if (i.eq.j) Gloc(i,im,ispin)=Gloc(i,im,ispin)
     1                            +DMFT_UU*kweight(nkp)
                      DMFT_U0=evec0(i,nb)*conjg(evec0(j,nb))*Gs0
                      Gsum(i,j)=Gsum(i,j)+T*(DMFT_UU-DMFT_U0)*
     1                          sweight*kweight(nkp)
                      Gsum(j,i)=Gsum(j,i)+T*conjg(DMFT_UU-DMFT_U0)*
     1                          kweight(nkp)*sweight
                    enddo
                  enddo
                enddo
              enddo
     ...
     1                        ))*kweight(nkp)*sweight
              ...
              do i=1,num_wann
                ...
                if (ispin.eq.1 .AND. nspin.eq.2) then 
                  mom(i)=mom(i)+real(Gsum(i,i))
                endif
                if (ispin.eq.2) mom(i)=mom(i)-real(Gsum(i,i))
                do j=1,num_wann
              ...
              do im=noms+1,nom
                ...
                Ginv=hk*(-1.0)
                do j=1,num_wann
                  Ginv(j,j)=Ginv(j,j)+mu+dcmplx(0.0,SigMdc(1,im,ispin))
     ...
     1               num_wann,INFO)
                do i=1,num_wann
                  Gloc(i,im,ispin)=Gloc(i,im,ispin)+id(i,i)*kweight(nkp)
                enddo
              enddo
            enddo
          endif    
        enddo
        tot_Gloc=0;tot_dm=0;tot_mom=0;tot_Ekin=0;tot_Ed=0
        CALL MPI_REDUCE(Gloc,tot_Gloc,num_wann*nom*nspin,
     ...
     1        MPI_SUM,MPI_Comm_World,ierr)
        CALL MPI_ALLREDUCE(mom,tot_mom,num_wann,MPI_DOUBLE_PRECISION,
     1        MPI_SUM,MPI_Comm_World,ierr)
        !write(*,*) ncor_orb,norb,tot_dm
!        allocate(Sig(2*ncor_orb))
!        allocate(dm_ud(2*ncor_orb))
!        allocate(old_Sig(ncor_orb))
!        allocate(new_Sig(ncor_orb))
        Epot_st=0;Epot_dy=0;!Sig_st=0;Sig=0;new_Sig=0;
        do i=1,ncor_orb!/norb
!          Nd=0
!          do j=1,2*norb
...
!          do j=1,norb
!            !UC(i,j,j)=UC(i,j,j)-U(i)
!            dm_ud((i-1)*2*norb+j)=(tot_dm((i-1)*norb+j)
!     1          +tot_mom((i-1)*norb+j))
!          enddo
!          do j=1,norb
!            !UC(i,j,j)=UC(i,j,j)-U(i)
!            dm_ud((i-1)*2*norb+norb+j)=(tot_dm((i-1)*norb+j)
!     1          -tot_mom((i-1)*norb+j))
!          enddo
!          do j=1,2*norb
!            do l=1,2*norb
!              if (j.eq.l) then
!                Sig((i-1)*2*norb+j)=Sig((i-1)*2*norb+j)+UC(i,j,l)
!     1          *dm_ud((i-1)*2*norb+l)
!              else
!                Sig((i-1)*2*norb+j)=Sig((i-1)*2*norb+j)+(UC(i,j,l)+U(i))
!     1          *dm_ud((i-1)*2*norb+l)
!              endif
!            enddo
          Epot_st=Epot_st+SigMdc(2*i,nom,1)*(tot_dm(i)+tot_mom(i))/2
          if (nspin.eq.2) then
             Epot_st=Epot_st+SigMdc(2*i,nom,2)*(tot_dm(i)-tot_mom(i))/2
          else
             Epot_st=Epot_st+SigMdc(2*i,nom,1)*(tot_dm(i)-tot_mom(i))/2
          endif
!            Epot_st=Epot_st+Sig((i-1)*2*norb+j)*dm_ud((i-1)*2*norb+j)/2
!            Sig((i-1)*2*norb+j)=Sig((i-1)*2*norb+j)
!     1                     -(Up(i)*(Nd-0.5)-Jh(i)*(Nd-1)/2)
!
!            Sig_st((i-1)*2*norb+j)=Sig_st((i-1)*2*norb+j)
!     1                     -(Up(i)*(Nd-0.5)-Jh(i)*(Nd-1)/2)
...
!          enddo
!        enddo
        tot_mom=tot_mom*2;tot_dm=tot_dm*2;tot_Ekin=tot_Ekin*2
...
!        deallocate(new_Sig)
      ...
      if (rank.eq.0) then
        do ispin=1,nspin
          do i=1,ncor_orb
            Sig0=SigMdc(2*i,nom,ispin)
            Sig1=SigMdc(2*i+1,nom,ispin)*SigMdc(1,nom,ispin)
            do j=1,nom
              Epot_dy=Epot_dy+2.0*T*real(tot_Gloc(i,j,ispin)
     1      *(dcmplx(SigMdc(2*i,j,ispin)-Sig0,SigMdc(2*i+1,j,ispin)))
     1         -Sig1/(SigMdc(1,j,ispin)**2))*sweight
            enddo
            Epot_dy=Epot_dy+Sig1/(4*T)*sweight
          enddo
        enddo
        allocate(write_Gloc(2*ncor_orb+1,nom,nspin))
        do ispin=1,nspin
          do im=1,nom
            write_Gloc(1,im,ispin)=SigMdc(1,im,ispin)
            do i=1,ncor_orb
              write_Gloc(2*i,im,ispin)=real(tot_Gloc(i,im,ispin))
              write_Gloc(2*i+1,im,ispin)=aimag(tot_Gloc(i,im,ispin))
            enddo
          enddo
        enddo
 ...
 110    FORMAT('(',I2,'F18.12)')
        ...
        !write(*,*) write_format
        DO im=1,nom
          WRITE(99,write_format) (write_Gloc(i,im,1),i=1,2*ncor_orb+1)
        ENDDO
        CLOSE(99)
        if (nspin.eq.2) then
          OPEN(UNIT=99,FILE='G_loc_dn.out',FORM='FORMATTED')
 111      FORMAT('(',I2,'F18.12)')
          WRITE(write_format,111) 2*ncor_orb+1
          !write(*,*) write_format
          DO im=1,nom
            WRITE(99,write_format) (write_Gloc(i,im,2),i=1,2*ncor_orb+1)
          ENDDO
          CLOSE(99)
        endif
...
!        CLOSE(99)
 ...
 132    FORMAT('(',I2,'F18.12)')
        ...
        WRITE(99,write_format,advance='no') (tot_dm(i),i=1,ncor_orb)
        WRITE(99,write_format) (tot_mom(i),i=1,ncor_orb)
        CLOSE(99)
     ...
     1   SigMdc(2,nom,1), SigMdc(2*ncor_orb,nom,1)  
        CLOSE(99)
      endif
